...
allowed = function(url, parenturl)
  if string.match(url, "'+")
      or string.match(url, "[<>\\%*%$;%^%[%],%(%)]")
      or string.match(url, "//$")
      or (parenturl ~= nil and string.match(parenturl, "^https?://[^/]*users%.content%.ytmnd%.com")) then
    return false
  end

  if string.match(url, "^https?://[^/]*content%.ytmnd%.com")
      or ids[string.match(url, "^https?://([^%.]+)")]
      or string.match(url, "^https?://[^%/]+%.ytmnd%.com/info/[0-9]+/json$") then
    return true
  end
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  if allowed(url, nil)  then
    ...
    if string.match(url, "^https?://[^%.]+%.ytmnd%.com/info/[0-9]+/json$") then
      ...
      check("http://" .. domain_start .. ".ytmnd.com")
      check(string.match(url, "^(https?://)") .. domain_start .. string.match(url, "^https?://[^%.]+(%..+)$"))
    end
